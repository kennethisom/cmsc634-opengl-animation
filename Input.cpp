...
void Input::keyPress(GLFWwindow *win, int key, AppContext *appctx)
{
    ...
    case 'P':                   // change "modes"
		switch(next_state) {
			case MOVER_WITH_CONTROLS:
				appctx->terrain->visible = false;
				appctx->lightmarker->visible = false;
				appctx->movers->showControls();
				appctx->movers->cloud = false;
				appctx->movers->orbit = false;
				next_state = MOVER_ONLY;
				break;
			case MOVER_ONLY:
				appctx->terrain->visible = false;
				appctx->lightmarker->visible = false;
				appctx->movers->hideControls();
				appctx->movers->cloud = false;
				appctx->movers->orbit = false;
				next_state = MOVER_CLOUD;
				break;
			case MOVER_CLOUD:
				appctx->terrain->visible = false;
				appctx->lightmarker->visible = false;
				appctx->movers->hideControls();
				appctx->movers->cloud = true;
				appctx->movers->orbit = false;
				next_state = MOVER_ORBIT;
				break;
			case MOVER_ORBIT:
				appctx->terrain->visible = true;
				appctx->lightmarker->visible = true;
				appctx->movers->hideControls();
				appctx->movers->cloud = false;
				appctx->movers->orbit = true;
				next_state = MOVER_WITH_CONTROLS;
				break;
		}
		redraw = true;
        break;
